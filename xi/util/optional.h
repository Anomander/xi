...
inline namespace ext {
  ...
  namespace detail {
    template < class T > struct optional_of { using type = optional< T >; };
    template < class T > struct optional_of< optional< T > > {
      using type = optional< T >;
    };
  }

  template < class T > using opt = typename detail::optional_of< T >::type;
  ...
  public:
    ...
    template < class U > auto and_(optional< U > &&optb) -> opt< U >;
  ...
  namespace detail {
    template < class T > struct _some {
      static opt< T > apply(T &&t) { return optional< T >(forward< T >(t)); }
    };
    template < class T > struct _some< optional< T > > {
      static opt< T > apply(optional< T > &&t) { return move(t); }
    };
  }
  ...
  template < class T > auto optional< T >::as_mut() {
    if (is_none()) { return optional< mut< T > >(none); }
    return optional< mut< T > >(edit(ext::get< T >(_value)));
  }

  template < class T > auto optional< T >::as_ref() {
    if (is_none()) { return optional< ref< T > >(none); }
    return optional< ref< T > >(val(ext::get< T >(_value)));
  }

  template < class T > T optional< T >::unwrap() {
    panic_if_none("unwrap");
    return move(extract_value());
  }

  template < class T > T optional< T >::unwrap_or(T &&def) {
  ...
  auto optional< T >::map_or(U &&def, F &&f) -> U {
    if (is_none()) { return forward< result_of_t< F(T && ) > >(def); }
  ...
  template < class T > optional< T >::operator bool() const { return is_some(); }

  /// PRIVATE METHODS
  template < class T > void optional< T >::panic_if_none(const char *method) {
    if (is_none()) { throw optional_panic(method); }
  }
  template < class T > void optional< T >::reset() {
    // A hack to reassign value to variant with a reference type.
    _value.~variant();
    new (&_value) variant_t(none);
  }
  template < class T > T optional< T >::extract_value() {
    XI_SCOPE(exit) { reset(); };
