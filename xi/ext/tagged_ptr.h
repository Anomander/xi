...
inline namespace ext {
  namespace detail {
    template < unsigned > struct bit_counter;

    template <> struct bit_counter< 4 > {
      enum { count = 2, mask = ~3 };
    };

    template <> struct bit_counter< 8 > {
      enum { count = 3, mask = ~7 };
    };
  }

  template < class T, class E > class tagged_ptr {
    using bit_counter = detail::bit_counter< sizeof(void*) >;
    template < E tag > constexpr void verify_bit() noexcept {
  ...
  public:
    ...
    template < E tag > void set_tag() {
      verify_bit< tag >();
      _pointer = reinterpret_cast< T* >(uintptr_t(_pointer) | tag);
    }

    template < E tag > void reset_tag() {
      verify_bit< tag >();
      _pointer = reinterpret_cast< T* >(uintptr_t(_pointer) & ~tag);
    }

    template < E tag > E get_tag() {
      verify_bit< tag >();
