...
#include "xi/async/future/shared_state.h"
...
namespace async {
  ...
  template < class T > class future {
    ...
    template < class U > friend future< U > make_ready_future(U &&value);
    friend future<> make_ready_future();
    template < class U > friend future< U > make_ready_future(exception_ptr ex);
    ...
    own< shared_state< T > > extract_into_shared_state(
      ...
      if (is_ready()) {
        ...
        return move(st);
      } else { return ext::get< own< shared_state< T > > >(_state); }
    }
  ...
  public:
    ...
    bool is_ready() const { return apply_visitor(is_ready_visitor{}, _state); }
    bool is_exception() const {
      return apply_visitor(is_exception_visitor{}, _state);
    }

    template < class func > decltype(auto) then(func &&f) {
      auto *local = ext::get< state< T > >(&_state);
      if (local) { return local->set_continuation(forward< func >(f)); }
    ...
    decltype(auto) then(mut< core::executor > e, func &&f) {
      auto *local = ext::get< state< T > >(&_state);
      if (local) { return local->set_continuation(e, forward< func >(f)); }
      return ext::get< own< shared_state< T > > >(_state)
          ->set_continuation(e, forward< func >(f));
    }
  };

  template <> class future< void > : public future<> {
  public:
    using future<>::future;
  };

  template < class T > future< T > make_ready_future(T &&value) {
    return future< T >(ready_made, forward< T >(value));
  }
  future<> make_ready_future() { return future<>(ready_made, meta::null{}); }
  future<> make_ready_future(meta::null) { return make_ready_future(); }
  template < class T > future< T > make_ready_future(exception_ptr ex) {
