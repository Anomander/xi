  ...
  namespace libevent {
    namespace detail {
      ...
      inline event_state short_to_event_state(short state) noexcept {
        ...
        if (state & EV_READ) { result = (event_state)(result | kRead); }
        if (state & EV_WRITE) { result = (event_state)(result | kWrite); }
        if (state & EV_TIMEOUT) { result = (event_state)(result | kTimeout); }
