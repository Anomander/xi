...
namespace core {
  ...
  template < class T >
  struct unwrapped_result_t {
    using type = T;
  };

  template < class T >
  struct unwrapped_result_t< future< T > > {
    using type = typename unwrapped_result_t< T >::type;
  };

  template <>
  struct unwrapped_result_t< void > {
  ...
  template < class F >
  struct future_result_t< F, void > {
    using type = typename unwrapped_result_t< result_of_t< F() > >::type;
  };

  template < class F >
  struct future_result_t< F, meta::null > {
    using type = typename unwrapped_result_t< result_of_t< F() > >::type;
  };
  ...
  STATIC_ASSERT_TEST(
      is_same< future_result< void (*)(int, double), int, double >,
               meta::null >);
  STATIC_ASSERT_TEST(
