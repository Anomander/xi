...
namespace core {
  ...
  namespace memory {
    ...
    static constexpr usize page_bits      = 12;
    static constexpr usize page_size      = 1 << page_bits;  // 4K
    static constexpr usize huge_page_size = 512 * page_size; // 2M

    void configure(vector<resource> m,
            opt<string> hugetlbfs_path = {});

    void* allocate_reclaimable(usize size);
    ...
    public:
      ...
      reclaimer(reclaim_fn reclaim,
                reclaimer_scope scope = reclaimer_scope::async);
      ~reclaimer();
      reclaiming_result do_reclaim() {
        return _reclaim();
      }
