...
// by size.  When spans are broken up or coalesced, they may move into new
// lists.
...
namespace core {
  namespace memory {
    using mmap_area = unique_ptr< char[], function< void(void*) > >;
    ...
    mmap_area mmap_anonymous(void* addr, size_t length, int prot, int flags) {
      ...
      printf("mmap addr %p\n", addr);
      printf("mmap ret %p\n", ret);
      return mmap_area(reinterpret_cast< char* >(ret), [length](void* p) {
        printf("unmap %p\n", p);
        ::munmap(p, length);
      });
    }
    ...
    alignas(64) static thread_local u64 g_allocs;
    static thread_local u64 g_frees;
    static thread_local u64 g_cross_cpu_frees;
    static thread_local u64 g_reclaims;

    using allocate_system_memory_fn =
        function< mmap_area(opt< void* >, usize) >;
    ...
    struct page {
      ...
      u8 offset_in_span;
      u16 nr_small_alloc;
      u32 span_size; // in pages, if we're the head or the tail
      page_list_link link;
    ...
    public:
      ...
      page* find(u32 n_pages, page* ary) {
        auto n = _front;
    ...
    private:
      usize span_bytes() const {
        return _span_size * page_size;
      }
    ...
    public:
      ...
      static constexpr unsigned usizeo_idx(unsigned size);
    ...
    constexpr unsigned small_pool::usizeo_idx(unsigned size) {
    ...
    public:
      static constexpr unsigned nr_small_pools =
          small_pool::usizeo_idx(4 * page_size) + 1;
    ...
    static constexpr usize max_small_allocation =
    ...
    struct cpu_pages {
      ...
      union pla {
        ...
        page_list free_spans[nr_span_lists]; // contains spans with span_size >=
                                             // 2^idx
      } fsu;
      small_pool_array small_pools;
      alignas(
          cache_line_size) std::atomic< cross_cpu_free_item* > xcpu_freelist;
      ...
      void free_span(pageidx start, u32 nr_pages);
      void free_span_no_merge(pageidx start, u32 nr_pages);
      void* allocate_small(unsigned size);
      void free(void* ptr);
      void free(void* ptr, usize size);
      void shrink(void* ptr, usize new_size);
      void free_cross_cpu(unsigned cpu_id, void* ptr);
      bool drain_cross_cpu_freelist();
      usize object_size(void* ptr);
      page* to_page(void* p) {
      ...
      void resize(usize new_size, allocate_system_memory_fn alloc_sys_mem);
      void do_resize(usize new_size, allocate_system_memory_fn alloc_sys_mem);
    ...
    alignas(64) static thread_local cpu_pages cpu_mem;
    ...
    static inline unsigned index_of_conservative(unsigned pages) {
      if (pages == 1) {
        return 0;
      }
      return std::numeric_limits< unsigned >::digits -
    ...
    void cpu_pages::free_span_no_merge(u32 span_start, u32 nr_pages) {
      assert(nr_pages);
    ...
    void cpu_pages::free_span(u32 span_start, u32 nr_pages) {
    ...
    page* cpu_pages::find_and_unlink_span_reclaiming(unsigned n_pages) {
      while (true) {
        ...
        if (run_reclaimers(reclaimer_scope::sync) ==
            reclaiming_result::reclaimed_nothing) {
          return nullptr;
        }
      }
    }
    ...
    void* cpu_pages::allocate_large(unsigned n_pages) {
      return allocate_large_and_trim(n_pages,
                                     [n_pages](unsigned idx, unsigned n) {
                                       return trim{0, std::min(n, n_pages)};
                                     });
    }

    void* cpu_pages::allocate_large_aligned(unsigned align_pages,
                                            unsigned n_pages) {
      return allocate_large_and_trim(
          n_pages + align_pages - 1, [=](unsigned idx, unsigned n) {
            return trim{align_up(idx, align_pages) - idx, n_pages};
          });
    }

    void* cpu_pages::allocate_small(unsigned size) {
      auto idx   = small_pool::usizeo_idx(size);
    ...
    usize cpu_pages::object_size(void* ptr) {
      pageidx idx = (reinterpret_cast< char* >(ptr) - mem()) / page_size;
      page* span  = &pages[idx];
      if (span->pool) {
        return span->pool->object_size();
      } else {
        return usize(span->span_size) * page_size;
      }
    }
    ...
    void cpu_pages::free(void* ptr, usize size) {
      ...
      if (size <= max_small_allocation) {
        auto pool = &small_pools[small_pool::usizeo_idx(size)];
        pool->deallocate(ptr);
      } else {
        free_large(ptr);
      }
    ...
    void cpu_pages::shrink(void* ptr, usize new_size) {
      auto obj_cpu = object_cpu_id(ptr);
      ...
      usize new_size_pages = align_up(new_size, page_size) / page_size;
    ...
    bool cpu_pages::initialize() {
      ...
      auto base        = mem_base() + (usize(cpu_id) << cpu_id_shift);
      ...
      // the last span.
      auto reserved =
          align_up(sizeof(page) * (nr_pages + 1), page_size) / page_size;
    ...
    mmap_area allocate_anonymous_memory(opt< void* > where, usize how_much) {
      auto flags = MAP_PRIVATE | (where ? MAP_FIXED : 0);
      auto addr  = where.unwrap_or((void*)nullptr);
      // printf("ADDR: %p\n", addr);
      // printf("FLAGS: %d\n", flags);
      return mmap_anonymous(addr, how_much, PROT_READ | PROT_WRITE, flags);
    }

    // mmap_area allocate_hugetlbfs_memory(file_desc& fd,
    //                                     opt< void* > where,
    //                                     usize how_much) {
    //   auto pos = fd.size();
    //   fd.truncate(pos + how_much);
    //   auto ret = fd.map(how_much,
    //                     PROT_READ | PROT_WRITE,
    ...
    // void cpu_pages::replace_memory_backing(
    //     allocate_system_memory_fn alloc_sys_mem) {
    //   // We would like to use ::mremap() to atomically replace the old
    //   anonymous
    //   // memory with hugetlbfs backed memory, but mremap() does not support
    //   // hugetlbfs
    //   // (for no reason at all).  So we must copy the anonymous memory to
    //   some
    //   // other
    //   // place, map hugetlbfs in place, and copy it back, without modifying
    //   it
    //   // during
    //   // the operation.
    //   auto bytes   = nr_pages * page_size;
    //   auto old_mem = mem();
    //   auto relocated_old_mem =
    //       mmap_anonymous(nullptr, bytes, PROT_READ | PROT_WRITE,
    //       MAP_PRIVATE);
    //   ::memcpy(relocated_old_mem.get(), old_mem, bytes);
    //   alloc_sys_mem(some((void*)old_mem), bytes);
    //   ::memcpy(old_mem, relocated_old_mem.get(), bytes);
    // }

    void cpu_pages::do_resize(usize new_size,
                              allocate_system_memory_fn alloc_sys_mem) {
      ...
      // printf("resize to %p\n", memory + old_size);
      auto mem = alloc_sys_mem(some((void*)mmap_start), mmap_size);
      memset((u8*)mem.get() + 100, '0', 1024);
      memset((u8*)mem.get() + 100, 0, 1024);
      mem.release();
    ...
    void cpu_pages::resize(usize new_size,
                           allocate_system_memory_fn alloc_memory) {
    ...
    reclaiming_result cpu_pages::run_reclaimers(reclaimer_scope scope) {
      ...
      while (nr_free_pages < target) {
        ...
        for (auto&& r : reclaimers) {
          if (r->scope() >= scope) {
            made_progress |=
                r->do_reclaim() == reclaiming_result::reclaimed_something;
          }
        }
        if (!made_progress) {
          return result;
    ...
    void cpu_pages::set_reclaim_hook(
        std::function< void(std::function< void() >) > hook) {
      reclaim_hook           = hook;
      current_min_free_pages = min_free_pages;
    }
    ...
    // Should not throw in case of running out of memory to avoid infinite
    // recursion,
    // becaue throwing std::bad_alloc requires allocation.
    ...
    void small_pool::add_more_objects() {
      ...
      while (_free_count < goal) {
        auto data =
            reinterpret_cast< char* >(cpu_mem.allocate_large(_span_size));
        if (!data) {
          return;
        }
        ++_spans_in_use;
        ...
        for (unsigned offset = 0; offset <= span_bytes() - _object_size;
             offset += _object_size) {
    ...
    void abort_on_underflow(usize size) {
      if (std::make_signed_t< usize >(size) < 0) {
    ...
    void* allocate_large(usize size) {
      ...
      assert((usize(size_in_pages) << page_bits) >= size);
      return cpu_mem.allocate_large(size_in_pages);
    }

    void* allocate_large_aligned(usize align, usize size) {
      abort_on_underflow(size);
      unsigned size_in_pages  = (size + page_size - 1) >> page_bits;
    ...
    void* allocate_aligned(usize align, usize size) {
      ++g_allocs;
    ...
    void free(void* obj, usize size) {
      ++g_frees;
      cpu_mem.free(obj, size);
    }

    void shrink(void* obj, usize new_size) {
      ++g_frees;
    ...
    void configure(std::vector< resource > m,
                   opt< std::string > hugetlbfs_path) {
      usize total = 0;
      for (auto&& x : m) {
        total += x.bytes;
      }
      ...
      //   auto fdp =
      //       make_lw_shared< file_desc
      //       >(file_desc::temporary(*hugetlbfs_path));
      //   sys_alloc = [fdp](opt< void* > where, usize how_much) {
      ...
      for (auto&& x : m) {
        ...
        if (r == -1) {
          ...
          std::cerr << "WARNING: unable to mbind shard memory; performance may "
                       "suffer: "
                    << err << std::endl;
        }
#endif // XI_HAS_NUMA
...
using namespace xi::core::memory;
using xi::usize;

extern "C"[[gnu::visibility("default")]][[gnu::externally_visible]] void*
malloc(usize n) throw() {
  if (n == 0) {
    return nullptr;
  }
  try {
...
extern "C"[[gnu::alias("malloc")]][[gnu::visibility("default")]] void*
__libc_malloc(usize n) throw();

extern "C"[[gnu::visibility("default")]][[gnu::externally_visible]] void
free(void* ptr) {
  if (ptr) {
    xi::core::memory::free(ptr);
  }
}

extern "C"[[gnu::alias("free")]][[gnu::visibility("default")]] void*
__libc_free(void* obj) throw();

extern "C"[[gnu::visibility("default")]] void*
calloc(usize nmemb, usize size) {
...
extern "C"[[gnu::alias("calloc")]][[gnu::visibility("default")]] void*
__libc_calloc(usize n, usize m) throw();
...
realloc(void* ptr, usize size) {
  ...
  if (size < old_size) {
    xi::core::memory::shrink(ptr, size);
    return ptr;
  }
...
extern "C"[[gnu::alias("realloc")]][[gnu::visibility("default")]] void*
__libc_realloc(void* obj, usize size) throw();

extern "C"[[gnu::visibility("default")]][[gnu::externally_visible]] int
posix_memalign(void** ptr, usize align, usize size) {
  try {
...
extern "C"[[gnu::alias("posix_memalign")]][[gnu::visibility("default")]] int
__libc_posix_memalign(void** ptr, usize align, usize size) throw();

extern "C"[[gnu::visibility("default")]] void*
memalign(usize align, usize size) {
  try {
...
extern "C"[[gnu::visibility("default")]] void*
aligned_alloc(usize align, usize size) {
  try {
    return allocate_aligned(align, size);
  } catch (std::bad_alloc&) {
    return NULL;
...
extern "C"[[gnu::alias("memalign")]][[gnu::visibility("default")]] void*
__libc_memalign(usize align, usize size);
...
extern "C"[[gnu::alias("cfree")]][[gnu::visibility("default")]] void
...
[[gnu::visibility("default")]] void
operator delete(void* ptr, usize size) throw() {
  if (ptr) {
    xi::core::memory::free(ptr, size);
  }
}
...
[[gnu::visibility("default")]] void*
operator new(usize size, std::nothrow_t) throw() {
  if (size == 0) {
    size = 1;
  }
  try {
...
[[gnu::visibility("default")]] void
operator delete(void* ptr, std::nothrow_t) throw() {
  if (ptr) {
    xi::core::memory::free(ptr);
  }
}
...
[[gnu::visibility("default")]] void
operator delete(void* ptr, usize size, std::nothrow_t) throw() {
  if (ptr) {
    xi::core::memory::free(ptr, size);
  }
}
...
namespace core {
  namespace memory {
    ...
    void configure(vector< resource > m, opt< std::string > hugepages_path) {
    }
    ...
    // translation
    // translate(const void* addr, usize size) {
    //   return {};
    // }
