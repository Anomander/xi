...
namespace async {
  namespace libevent {
    ...
    event_loop::~event_loop() { event_base_free(_event_base); }
    ...
    void event_loop::event_callback(int descriptor, short what,
                                    void *arg) noexcept {
      if (arg) {
    ...
    struct ::event *event_loop::prepare_event(
        struct ::event *e, event_state state,
        xi::async::event_handler *arg) noexcept {
      auto s = detail::event_state_to_short(state);
      if (e == nullptr) {
        return event_new(_event_base, arg->descriptor(), s,
                         &event_loop::event_callback, arg);
      } else {
        event_assign(e, _event_base, arg->descriptor(), s,
                     &event_loop::event_callback, arg);
        return e;
      }
    }
