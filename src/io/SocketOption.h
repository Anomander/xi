...
namespace xi {
  namespace io {
    ...
    struct BooleanSocketOption : SocketOption <int, Level, Name> {
      using Base = SocketOption <int, Level, Name>;

      using Base::SocketOption;

      BooleanSocketOption(bool value)
        : Base (value ? 1 : 0)
      {}
    ...
    template<int Level, int Name>
    BooleanSocketOption<Level, Name> BooleanSocketOption<Level, Name>::yes(true);
    template<int Level, int Name>
    BooleanSocketOption<Level, Name> BooleanSocketOption<Level, Name>::no(false);
