...
public:
  HandlerContext(mut< Channel > channel, own< PipelineHandler > handler) : _channel(channel), _handler(move(handler)) {}

  template < class Event > void fire(Event e) { _handler->onEvent< Event >(this, move(e)); }

  template < class Event > void forward(Event e) { _forward(edit(e), move(e)); }
...
private:
  template < class Event > void _forward(mut< UpstreamEvent >, Event e) {
    if (_nextRead) {
      _nextRead->fire(move(e));
    }
  }
  template < class Event > void _forward(mut< DownstreamEvent >, Event e) {
    if (_nextWrite) {
