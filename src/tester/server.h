...
public:
  ...
  void schedule_read() {
    async_read(socket_, buffer(data_, 0),
               boost::bind(&session::handle_read_event, this, placeholders::error, placeholders::bytes_transferred));
  }
  void handle_read_event(boost::system::error_code error, size_t bytes_transferred) {
    while (!error) {
      auto data = make_shared< vector< char > >(max_length);
      bytes_transferred = socket_.receive(buffer(*data), MSG_DONTWAIT, error);
      if (bytes_transferred > 0) {
        async_write(socket_, buffer(*data, bytes_transferred), [data](auto error, size_t) {});
      }
