...
template < typename T > struct FastCastableNode {
...
template < class Group > struct FastCastable;
...
template < class... Ts > struct FastCastableGroup : FastCastable< meta::vector< Ts... > > {
  using FastCastableGroupTypes = meta::vector< Ts... >;
};

template < typename T > T* fast_cast(FastCastableNode< T >* src) { return src->cast(); }
template < typename T > T const* fast_cast(FastCastableNode< T > const* src) { return src->cast(); }
template < typename T > shared_ptr< T > fast_cast(shared_ptr< FastCastableNode< T > > src) {
  auto* cast = src->cast();
  if (nullptr != cast) {
...
template < typename T > auto consuming_fast_cast(shared_ptr< FastCastableNode< T > > src) {
  return fast_cast< T >(move(src));
}
template < typename T > unique_ptr< T > consuming_fast_cast(unique_ptr< FastCastableNode< T > > src) {
  auto* cast = src->cast();
  if (nullptr != cast) {
...
template < class D, class Group, class Types > struct FastCastGroupMemberImpl;
...
private:
  template < class Src > static auto _doCast(Src* src, meta::TrueType) { return static_cast< D* >(src); }
  template < class Src > static auto _doCast(Src const* src, meta::TrueType) { return static_cast< D const* >(src); }
  template < class Src > static auto _doCast(Src*, meta::FalseType) { return nullptr; }
