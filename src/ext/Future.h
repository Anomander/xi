...
#define METHOD_FORWARD(name, method)                                                                                   \
  template < class... Args > auto name(Args&&... args) { return method(forward< Args >(args)...); }

template < class T > struct Future : public ::boost::future< T > {
  using Base = ::boost::future< T >;
  using Base::future;
  Future(Base&& f) : Base(::std::forward< Base >(f)) {}
};

template < class T > struct Promise : public ::boost::promise< T >, virtual public ownership::StdShared {
